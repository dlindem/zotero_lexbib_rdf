...
if len(created) > 0:
    ...
    print('\nQuickstatements V1 item creation commands file written. Finished.\n')
    sys.exit()

print('\nDid not find any new items. Will not write QS item creation file.\nWill proceed looking at properties...\n')
            ...
            if triple['p'] in pll.values(): # checks in property is known in wikibase
                ...
                if re.match(r'https?://', triple['o']): # object is URL or URL
                    if triple['o'] in ill.values(): # if object is a known wikibase item...
                        object = get_key(ill, triple['o']).replace("http://lexbib.wiki.opencura.com/entity/", "") # replace with its Qid for QS v1 command
                        qscsv += object+"\n"
                    else:
                        qscsv += "\""+triple['o']+"\"\n"
                else:
                ...
                labels[subject]={property:triple['o']}
            else:
...
with open('D:/LexBib/wikibase/qsv1_edit_commands.txt', 'w', encoding='utf-8') as qsfile:
        qsfile.write(qscsv)

print('\nQuickstatements V1 edit commands file written. Finished.\n')
